import {$, $$, expect, driver} from "@wdio/globals"; //autocompletion supported from WebDriverIO9 and above
...
describe('iOS Find Element', () => {
    ...
    it.only('exercise search input field', async () => {
        ...
        await expect($(`-ios predicate string:${searchField}`)).toHaveText("I love this course");
        await $('~Clear text').click();
        await expect($(`-ios predicate string:${searchField}`)).not.toHaveText();     
    });
});